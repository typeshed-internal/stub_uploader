## [2.0.0.20260827](https://pypi.org/project/types-uWSGI/2.0.0.20260827/) (2026-08-27)

* fixed some typos ([#16273](https://github.com/python/typeshed/pull/16273))

## [2.0.0.20260518](https://pypi.org/project/types-uWSGI/2.0.0.20260518/) (2026-05-18)

* Upgrade black to 26.5.0 ([#15801](https://github.com/python/typeshed/pull/15801))

## [2.0.0.20260508](https://pypi.org/project/types-uWSGI/2.0.0.20260508/) (2026-05-08)

* Import some items from typing instead of typing_extensions ([#15711](https://github.com/python/typeshed/pull/15711))

    Part of #13782

## [2.0.0.20260408](https://pypi.org/project/types-uWSGI/2.0.0.20260408/) (2026-04-08)

* Use dashes instead of underscores for METADATA.toml field names ([#15614](https://github.com/python/typeshed/pull/15614))

## [2.0.0.20260331](https://pypi.org/project/types-uWSGI/2.0.0.20260331/) (2026-03-31)

* Use a `TypeVarTuple` in `uwsgidecorators.thread` ([#11128](https://github.com/python/typeshed/pull/11128))

## [2.0.0.20250809](https://pypi.org/project/types-uWSGI/2.0.0.20250809/) (2025-08-09)

* Mark stub-only private symbols as `@type_check_only` in third-party stubs ([#14545](https://github.com/python/typeshed/pull/14545))

## [2.0.0.20250801](https://pypi.org/project/types-uWSGI/2.0.0.20250801/) (2025-08-01)

* Split `tool.stubtest.platforms` metadata key ([#13746](https://github.com/python/typeshed/pull/13746))

    Co-authored-by: Avasam <samuel.06@hotmail.com> \
    Co-authored-by: pre-commit-ci[bot] <66853113+pre-commit-ci[bot]@users.noreply.github.com>

## [2.0.0.20240516](https://pypi.org/project/types-uWSGI/2.0.0.20240516/) (2024-05-16)

* uWSGI: Makes stubtest_allowlist_darwin more resilient against CI flakyness ([#11819](https://github.com/python/typeshed/pull/11819))

## [2.0.0.20240425](https://pypi.org/project/types-uWSGI/2.0.0.20240425/) (2024-04-25)

* Bump pyright to v1.1.360 ([#11810](https://github.com/python/typeshed/pull/11810))

* Disable uwsgi, pyaudio, jack-client stubtest on macOS ([#11821](https://github.com/python/typeshed/pull/11821))

## [2.0.0.20240414](https://pypi.org/project/types-uWSGI/2.0.0.20240414/) (2024-04-14)

* uwsgi: remove unused allowlist entry ([#11754](https://github.com/python/typeshed/pull/11754))

    Also wow, some of the logs that come from uwsgi are quite interesting

## [2.0.0.20240311](https://pypi.org/project/types-uWSGI/2.0.0.20240311/) (2024-03-11)

* Use PEP 570 syntax in third party stubs ([#11554](https://github.com/python/typeshed/pull/11554))

## [2.0.0.20240106](https://pypi.org/project/types-uWSGI/2.0.0.20240106/) (2024-01-06)

* Update typing_extensions imports in third-party stubs ([#11245](https://github.com/python/typeshed/pull/11245))

## [2.0.0.1](https://pypi.org/project/types-uWSGI/2.0.0.1/) (2023-11-24)

* Third-party stubs: remove unused `type: ignore`s ([#11063](https://github.com/python/typeshed/pull/11063))

## 2.0.0.0 (2023-07-24)

Adds stubs for uWSGI ([#10432](https://github.com/python/typeshed/pull/10432))

This adds stubs for the uWSGI Python API.

Similarly to GDB the Python API is only accessible within a uWSGI process, some parts of the API also only exist if certain configuration options are enabled. This makes running stubtest a bit of pain.

Co-authored-by: Alex Waygood <Alex.Waygood@Gmail.com>
Co-authored-by: Jelle Zijlstra <jelle.zijlstra@gmail.com>
Co-authored-by: Akuli <akuviljanen17@gmail.com>

